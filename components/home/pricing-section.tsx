    ...
    className="relative w-full max-w-lg hover:scale-105 hover:transition-all duration-300">
      ...
      >
        ...
        <MotionDiv variants={listVariants} className="flex gap-2">
          <div className="flex items-baseline gap-2">
            ...
            <div className="flex flex-col justify-end mb-[4px]">
              <span className="text-xs uppercase font-semibold">USD</span>
        ...
        <MotionDiv variants={listVariants} className="space-y-2.5 leading-relaxed text-base flex-1">
          {items.map((item, idx) =>
            <li key={idx} className="flex items-centerm gap-2">
              <CheckIcon size={18} />
              <span>
                {item}
              </span>
    ...
    className="relative overflow-hidden" id="pricing">
      <div className="py-12 lg:py-24 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 lg:pt-12">
        ...
        <div className="relative flex justify-center flex-col lg:flex-row items-center lg:items-stretch gap-8">
          {PricingPlans.map(plan => <PricingCard key={plan.id} {...plan} />)}
        </div>
      </div>
    </MotionSection>
  );
